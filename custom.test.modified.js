...
describe('Custom Tests', function () {
    ...
    it('[assignment] iii. see assignment doc for details', async () => {
        ...
        expect(aliceDepositUtxo.amount).to.be.equal(aliceDepositAmount)
        expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount)
        ...
        tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
        // Assert bob balance on L2
        expect(bobBalance).to.be.equal(bobWithdrawAmount)
        expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount.sub(bobWithdrawAmount))

        /**
         * Alice withdraws all to L1
         */
            // withdraws a part of his funds from the shielded pool
        const aliceWithdrawAmount = utils.parseEther('0.07')
        const recipient = '0xEba7a10Cf80C2BB37BEb3a618bB35f993c71161E'
        const aliceChange2Utxo = new Utxo({
        ...
        await transaction({
            ...
            outputs: [aliceChange2Utxo],
            recipient: recipient,
            isL1Withdrawal: true,
        })

        tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
        //pool balance after alice withdrawal must be zero again
